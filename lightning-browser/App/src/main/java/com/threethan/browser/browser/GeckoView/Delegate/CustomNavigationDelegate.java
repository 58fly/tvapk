...
public class CustomNavigationDelegate implements GeckoSession.NavigationDelegate {
    ...
    public void onLocationChange(@NonNull GeckoSession session, @Nullable String url,
        ...
        if (url != null && !url.isEmpty() && !url.equals("about:blank") && !url.startsWith("javascript:")) {
            currentUrl = url;
    ...
    public GeckoResult<AllowOrDeny> onLoadRequest(@NonNull GeckoSession session, @NonNull LoadRequest request) {
        if (request.uri.startsWith("javascript:")) {
        ...
        if (!mActivity.shouldKeepTopBarHidden()) mActivity.showTopBar();
